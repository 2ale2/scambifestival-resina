...
async def dispatcher(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if hasattr(update.callback_query, "data"):
        ...
        if for_who.__len__() > 1:
            ...
            if for_who[0] == "close":
                await utils.delete_message(context, chat_id=variables.CHAT_ID, message_id=int(for_who[1]))
                return

            if for_who[0] == "restrict" or for_who[0] == "not_restrict":
                user = context.bot_data[for_who[2]]
                if for_who[0] == "restrict":
                    await add_user(context, user, "LIMITED_USERS", for_who[1])
                    ...
                    message = await bot.sendMessage(chat_id=variables.CHAT_ID,
                                                    text="🔒 *Restriction*\n\n" + uid + "` has been"
                                                         ...
                                                         " mistake, you can ask @AleLntr to fix that for you.\n\n",
                                                    parse_mode='MARKDOWN')
                    ...
                    mid = user.last_mess.last_group_message.message_id
                    # noinspection PyUnboundLocalVariable
                    keyboard = [
                        [InlineKeyboardButton("Cancel", callback_data="close " + str(mid))]
                    ]
                    reply_markup = InlineKeyboardMarkup(keyboard)
        ...
        if for_who.__len__() == 1:
            ...
            if "flagged" in for_who:
                res = await db_functions.get_flagged_users(context, None)
                if res.__len__() > 0:
                    string = ""
                    c = 1
                    for i in res:
